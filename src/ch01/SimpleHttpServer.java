...
public class SimpleHttpServer {
	...
	public static void main(String[] args) {
		...
		// 8080 <- https, 80 <- http (포트번호 생략 가능하
		try {
			...
			// 프로토콜 정의 (경로, 핸들러 처리)
			httpServer.createContext("/test", new MyTestHandler()); // 핸들러 처리를 내부 정적 클래스로 사용
	...
	static class MyTestHandler implements HttpHandler {
		...
		public void handle(HttpExchange exchange) throws IOException {
			// 사용자의 요청 방식(MATHOD) , GET, POST 알아야 우리가 동작 시킬 수 있다.
...
//				System.out.println("여기는 POST 방식으로 호출 됨");
				...
				exchange.sendResponseHeaders(405, response.length()); // Metgod Not Allowed
